...
const BreadCrumbDesk = () => {
  const { categories, subCategories, brands }: any = useContext(categoryContext);
  const location = useLocation();
  const route = location.pathname.split("/");
  const id = location.pathname.split("/").pop();

  const brandOBJ = brands?.find(
    (_brand: any) => _brand.id == id
  );
  ...
  return (
    <>
      <section className="hidden desk:block max-w-whole text-right font-shabnam m-auto w-90% desklg:w-full pt-7">
        <Link
        ...
        {route[1] === "category" && route[2] === "subcategory" && (
          <>
            <span className="px-3 text-text">/</span>
            <Link className="text-text hover:text-primary transition-all duration-300" to={`/category/subcategory/${catOBJ?.id}`}>{catOBJ?.name}</Link>
          </>
        )}
        {route[1] === "category" && route.length === 3 && (
          <>
            ...
            <span className="px-3 text-text">/</span>
            <Link className="text-text hover:text-primary transition-all duration-300" to={`/category/${subCatOBJ?.id}`}>{subCatOBJ?.name}</Link>
          </>
        )}
        {route[1] === "brand" && (
          <>
